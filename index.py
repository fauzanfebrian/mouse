...
while True:
    ...
    # Draw All
    cv2.rectangle(
        img, (800, 70), (800 + 400, 70 + 100), (225, 225, 225), cv2.FILLED
    )
    ...
    if hands:
        ...
        lmLists = []
        for hand in hands:
            lmLists.append(hand["lmList"])

        if len(lmLists) > 1:
            length, _, img = detector.findDistance(
                lmLists[0][8], lmLists[1][8], img
            )
            print(length)
            x, y, _ = lmLists[0][8]

            # If clicked check which button and perform action
            if length < 70 and delayCounter == 0:
                for i, button in enumerate(buttonList):
                    if button.checkClick(x, y):
                        myValue = buttonListValues[int(i % 4)][
                            int(i / 4)
                        ]  # get correct number
                        if myValue == "=":
                            myEquation = f"{eval(myEquation):0.2f}"
                        else:
                            myEquation += myValue
                        delayCounter = 1
    ...
    cv2.putText(
        img, myEquation, (810, 130), cv2.FONT_HERSHEY_PLAIN, 3, (0, 0, 0), 3
    )
